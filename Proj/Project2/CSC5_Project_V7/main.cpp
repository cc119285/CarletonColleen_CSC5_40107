/* 
  ...
  Created on February 8, 2017, 4:25 PM
 ...
 * game. Alternatively, compete with another player to try to get the most 
 * points.
 */
...
bool victory(bool, int, int = 5); //Function that determines win/loss
void copy(int [],int [],int); //Copies array with points
void copy(vector<int>, int [], int); //Copies an array's values to a vector
...
int main(int argc, char** argv) {
    ...
    cout<<"earn the most points. Enter the number of players (1 or 2)."<<endl;
    cin>>numPlrs;
    while (numPlrs<1) {
        cout<<"Number of players must be at least one."<<endl; //Validate input
        cin>>numPlrs;
    }
    while (numPlrs>2) {
...
int valsrch(int ptsarry[], int POINTSZ, int val) {
    ...
    //Sort
    while (index<POINTSZ && !found) {
        if (ptsarry[index] == val) { // If the value is found
        found = true; // Set the flag 
        positon = index; // Record the value's subscript
        } 
        index++; // Go to the next element
    }
    return positon; // Return the position, or -1
}

void copy(vector<int> scores, int ptsarry[], int POINTSZ) {
    //Copy the points from the array to the vector
